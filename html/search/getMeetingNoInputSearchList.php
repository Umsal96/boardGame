<?php
// 검색 결과를 가져오는 코드긴 한데 검색 결과를 db 에 넣지 않고 가져옴
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) AS total FROM meeting_table
    JOIN member_table ON meeting_table.meeting_seq = member_table.meeting_seq
    WHERE meeting_table.meeting_name LIKE :search");
    ...
    $stmt1 = $conn->prepare("SELECT meeting_table.*
    FROM meeting_table
    JOIN member_table
    ...
    $stmt1->bindParam(':limit_num', $limit, PDO::PARAM_INT);

    $stmt1->execute();

    $result['data'] = $stmt1->fetchAll(PDO::FETCH_ASSOC);

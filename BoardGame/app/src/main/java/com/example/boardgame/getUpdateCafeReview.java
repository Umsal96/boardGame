...
public class getUpdateCafeReview extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        textView36.setText("카페 후기 작성 수정");

        backPage.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(getUpdateCafeReview.this, getCafe.class);
                intent.putExtra("cafeId", item.getCafe_seq());
            }
        });
...
//                String filePath = FileUtils.getPathFromUri(getApplicationContext(), imageUri);
    ...
    private void getGameReview(int reviewId){
        HttpUrl.Builder urlBuilder = HttpUrl.parse("http://3.38.213.196/cafeReview/getCafeReview.php").newBuilder();
            ...
            public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                if(response.isSuccessful()){
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            item = jtg.jsonGetToGetCafeReview(responseData);
